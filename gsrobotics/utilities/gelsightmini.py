...
class GelSightMini:
    ...
    def select_device(self, device_idx=None) -> None:
        ...
        try:
            ...
            print(f"Setting camera resolution to {self.target_width}x{self.target_height}")
    ...
    def start_recording(self, filepath: str = None) -> None:
        ...
        if filepath is None or not os.path.isdir(filepath):
            log_message("No valid file path provided. Creating a folder on the Desktop.")
            filepath = self.create_folder()


        filepath = os.path.join(filepath, f"device_{self.camera.device}.mp4")
