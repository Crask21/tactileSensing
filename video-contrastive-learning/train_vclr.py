...
def main(args, writer):
    ...
    # --------------------------------- modified --------------------------------- #
    if use_ddp:
        model = DistributedDataParallel(model, device_ids=[args.local_rank] if torch.cuda.is_available() else None,
    ...
    if use_ddp and not dist.is_initialized():
        ...
        logger.info(f"Process group initialized with backend '{backend}'")
...
if __name__ == '__main__':
    ...
    if use_ddp:
        backend = 'nccl' if torch.cuda.is_available() else 'gloo'
        dist.init_process_group(backend=backend, init_method='env://')
        print(f"Distributed initialized with backend '{backend}' on rank {local_rank}")
    else:
