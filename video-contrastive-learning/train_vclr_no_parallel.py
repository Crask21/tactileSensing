...
def main(args, writer):
    ...
    optimizer = torch.optim.SGD(model.parameters(),
                                lr=args.batch_size * 1 / 256 * args.base_lr,
    ...
    for epoch in range(args.start_epoch, args.epochs + 1):
        ...
        logger.info('epoch {}, total time {:.2f}, loss={}'.format(epoch, time.time()-t, loss))
...
def train_vclr(epoch, train_loader, model, model_ema, contrast, contrast_tsn, criterion, optimizer, scheduler, writer, args):
    ...
    for idx, (xq, x1, x2, x3, order_label, tsn_q, tsn_k) in enumerate(train_loader):
        ...
        # forward keys
        with torch.no_grad():
            x1_feat_inter, x1_feat_intra = model_ema(x1)
            x2_feat_inter, x2_feat_intra = model_ema(x2)
            x3_feat_inter, x3_feat_intra = model_ema(x3)
            x1_feat_inter_all = x1_feat_inter
            x2_feat_inter_all = x2_feat_inter
            x3_feat_inter_all = x3_feat_inter

            # tsn, o3n
            tsn_k_feat, o3n_k_feat = model_ema(tsn_k, tsn_mode=True)
            tsn_k_feat_all = tsn_k_feat
            o3n_k_feat_all = o3n_k_feat
...
if __name__ == '__main__':
    ...
    # Fallback rank values if not running with torchrun or torch.distributed.launch
    ...
    rank = dist.get_rank() if dist.is_initialized() else 0
    logger = setup_logger(output=args.output_dir, distributed_rank=rank, name="vclr")
    if rank == 0:
